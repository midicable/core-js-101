...
function memoize(func) {
  const cache = new Map();
  return (...args) => {
    const argsQuasiHash = JSON.stringify(args);
    if (!cache.has(argsQuasiHash)) {
      cache.set(argsQuasiHash, func(...args));
    }
    return cache.get(argsQuasiHash);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${func.name}(${argsList.join(',')}) ends`);
